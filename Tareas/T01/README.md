...
* `atrapar_programon_salvaje(programon_salvaje)`: se llama al lanzar la prograbola y permite capturar al programón según la probabilidad de acertarle. Si se captura, se agrega a la **Progradex** y el **PC**. En caso en que la cantidad de programones en el equipo del jugador sea menor a 6, el capturado se agrega además al equipo.
